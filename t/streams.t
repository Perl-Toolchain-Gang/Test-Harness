...
my $iterator  = $factory->make_iterator( \*DATA );
isa_ok $iterator, 'TAP::Parser::Iterator';
my $parser = TAP::Parser->new( { stream => $iterator } );
...
$iterator = $factory->make_iterator( [ split /\n/ => $tap ] );
ok $parser = TAP::Parser->new( { stream => $iterator } ),
