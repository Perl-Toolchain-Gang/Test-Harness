...
sub _analyze_iterator {
    ...
    $results->set_skip_all( $self->{skip_all} )
...
sub _analyze_line {
    ...
    if ($point) {
        ...
        if (   ( $point->number > 100_000 )
            && ( $point->number > ( $self->{max} || 100_000 ) ) )
        {
    ...
    $self->callback->( $self, $line, $linetype, $results )
      if $self->callback;
...
sub _switches {
    ...
    my @existing_switches
      = $self->_cleaned_switches( $Test::Harness::Switches,
