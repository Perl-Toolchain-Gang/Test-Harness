...
of choice it's easy to generate valid TAP. It looks like this:
  ...
  1..3 ok 1 - init OK ok 2 - opened file not ok 3 - appended to
  file
...
If I need a feature that prove doesn't provide I can easily write
my own test harness; one of the motives for the 3.00 rewrite of
Test::Harness was to make it easier to subclass and extend.
...
The important code in prove is just:
  ...
  my $app = App::Prove->new; $app->process_args(@ARGV); exit(
  $app->run ? 0 : 1 );
...
a source (TAP::Parser::Source, TAP::Parser::Source::Perl) or an
