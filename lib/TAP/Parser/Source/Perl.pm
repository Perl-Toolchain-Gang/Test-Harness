...
sub get_stream {
    ...
    # TODO extra_libs is vestigial now, so kill the related code.
    my @extra_libs;
...
=head3 C<filtered_inc>

 my @inc = $source->filtered_inc;

Get the parts of @INC which are changed from the stock list AND
preserve reordering of stock directories.

=for developer
This is somewhat vistigial, currently only used by Test::Harness and
maybe needs to live elsewhere.  Should maybe take two array-refs
(\@orig_inc, \@filter_this_inc).
...
sub filtered_inc {
    ...
    # TODO do these os-specific bits still work?
    if (IS_VMS) {
    ...
    # we need to walk the list and filter it to preserve changes in the
    # default ordering
    my @new_inc;
    my %seen;
    for my $dir (@inc) {
